...
let to_string_piece pc =
  "{ position: "
  ^ string_of_int pc.position
  ^ ", in_play: "
  ^ string_of_bool pc.in_play
  ^ "}"

let to_string_player pl =
  List.map to_string_piece pl |> String.concat "; "
...
let rec put_pieces_inplay piece_number (player : player) =
  match player with
  | [] -> []
  | h :: t ->
      if h.index = piece_number then put_piece_inplay h :: t
      else h :: put_pieces_inplay piece_number t
...
let rec pieces_able_to_play pl roll =
  match pl with
  | [] -> []
  | h :: t ->
      if h.in_play && h.position - roll >= 0 then
        h :: pieces_able_to_play t roll
      else pieces_able_to_play t roll

let pieces_left pl =
  let pc =
    try Some (next_piece pl) with
    | _ -> None
  in
  match pc with
  | Some _ -> true
...
let rec legal_moves_available player roll =
  ...
  | [] -> false
  | h :: t ->
      ((legal_move h roll && h.in_play)
      || (roll = 6 && pieces_not_in_play player <> []))
      || legal_moves_available t roll
